...
export class AiAgentService {
   ...
   */
  private buildSystemPrompt(doctorId: string): string {
    const backendUrl = this.config.get<string>('BACKEND_URL') || 'http://localhost:8000';

    return `You are a helpful AI assistant for Dr. Rocky Haque's medical practice.
...
REQUIRED FIELDS:
- doctor_id: "${doctorId}" (use the doctor ID from your configuration)
...
DO NOT include booking IDs or confirmation numbers in the summary.`;
  ...
  private buildAgentTools(doctorId: string) {
    const backendUrl = this.config.get<string>('BACKEND_URL') || 'http://localhost:8000';
    const webhookApiKey = this.elevenLabsApiKey;
    ...
    return [
      {
        ...
        url: `${backendUrl}/api/v1/ai-agent/webhook`,
        method: 'POST',
        headers: {
      ...
      },
      {
        ...
        url: `${backendUrl}/api/v1/ai-agent/save-transcription`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': webhookApiKey,
      ...
      },
      {
        ...
        url: `${backendUrl}/api/v1/ai-agent/create-task`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': webhookApiKey,
      ...
      },
      {
        ...
        url: `${backendUrl}/api/v1/ai-agent/query-kb`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': webhookApiKey,
  ...
  async createDoctorAgent(doctorId: string): Promise<string> {
    ...
    const agentConfig = {
      conversation_config: {
        agent: {
          prompt: {
            prompt: this.buildSystemPrompt(doctorId),
          },
          first_message: 'Hello! Thank you for calling. How can I help you today?',
          language: 'en',
        },
        tts: {
          // Using default voice settings as requested
        },
      },
      platform_settings: {
        webhook_url: `${backendUrl}/api/v1/ai-agent/webhook/post-call?doctor_id=${doctorId}`,
      },
    };
    ...
    try {
      ...
      if (!agentId) {
        throw new BadRequestException('Failed to create agent: No agent_id returned');
      }

      console.log(`✅ Created ElevenLabs agent for doctor ${doctorId}: ${agentId}`);
      return agentId;
    } catch (error) {
  ...
  async getDoctorAgent(doctorId: string) {
    ...
    if (!doctor.elevenlabsAgentId) {
      throw new NotFoundException('No agent found for this doctor');
    }
    ...
    try {
      ...
      throw new NotFoundException(
        `Failed to fetch agent: ${error.response?.data?.detail || error.message}`,
      );
    }
  }
  ...
  async updateDoctorAgent(doctorId: string, updates: any) {
    ...
    try {
      ...
      console.log(`✅ Updated ElevenLabs agent for doctor ${doctorId}`);
      return {
        success: true,
        agent: response.data,
      };
      ...
      throw new BadRequestException(
        `Failed to update agent: ${error.response?.data?.detail || error.message}`,
      );
    }
  }

  /**
   * Delete agent from ElevenLabs and clear database reference
   */
  async deleteDoctorAgent(doctorId: string) {
    ...
    try {
      ...
      // Clear the agent ID from database
      await this.prisma.doctor.update({
        where: { id: doctorId },
        data: { elevenlabsAgentId: null },
      });

      console.log(`✅ Deleted ElevenLabs agent for doctor ${doctorId}`);
      return {
        success: true,
      ...
      throw new BadRequestException(
        `Failed to delete agent: ${error.response?.data?.detail || error.message}`,
      );
    }
  }

  /**
   * Recreate agent (delete old one and create new one)
   */
  async recreateDoctorAgent(doctorId: string) {
    // Try to delete existing agent if it exists
    try {
      await this.deleteDoctorAgent(doctorId);
    } catch (error) {
      // If agent doesn't exist, just clear the DB field
      await this.prisma.doctor.update({
        where: { id: doctorId },
        data: { elevenlabsAgentId: null },
      });
